...
| [`02-github-prs/`](02-github-prs/) | **Active — fully planned** | GitHub PR review queue. PAT auth, viewer header + three tabs (`Mine` / `Review` / `All`), date-grouped cards with author→reviewer avatar chains, per-repo toggle, hide PR, background sync via `chrome.alarms`. |
